...
else 
{
	...
	if (isset($_POST["submit"]))
	{
		...
		// update the survey stage in the database
		$nextStage = $_POST["NextSurveyStage"];
		$sql = "UPDATE user SET survey_stage = '$nextStage' WHERE user_id = '$intUserId'";
		$result = mysqli_query($conn, $sql)
			or die("Error: " .mysqli_error($conn));
	...
	if ($result = mysqli_query($conn, $sql))
	{
		...
		<div class="navbar navbar-fixed-top">
		 ...
		 <?php
			...
			<form class="form col-md-12 center-block" id = "secondForm" action ="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
			<?php
			
			echo "<h2 class=\"text-center heading\">Hospital</h2>";
			$stage = "HomeAndCommunity";
		...
		echo		"<input name='NextSurveyStage' value='$stage' type='hidden'/>";
		echo		"<input class='btn btn-primary submit' name='submit' value='Submit' type='submit' />"; 
		echo	"</div>";
		echo "</div>";	
		echo "</form>";
		echo "<script type=\"text/JavaScript\">";
		echo "$(\"#\"+\"secondForm\").validate()";
		echo "</script>";
		echo "</div>";
		echo "</div>";
		echo "</div>";
